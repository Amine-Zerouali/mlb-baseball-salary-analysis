...
\noindent\textit{Note : les $R^2$ sont calculés directement à partir des corrélations de
Pearson ($R^2 = r^2$) et confirmés par PROC REG.}
...
intrinsèque du joueur est moins valorisée que sa présence sur le terrain.
...
Le graphique des résidus en fonction des valeurs prédites révèle une légère
hétéroscédasticité : la dispersion des résidus est plus importante pour les
valeurs prédites faibles (joueurs peu actifs), ce qui est attendu car le
modèle peine à distinguer les contrats au plancher syndical. Le Q-Q plot
des résidus montre une bonne adéquation à la normalité sur la partie centrale,
avec quelques écarts dans les queues (outliers).
...
Le tableau~\ref{tab:outliers} présente les 6 joueurs dont le résidu positif
est le plus élevé, c'est-à-dire ceux dont le salaire dépasse le plus la
prédiction basée sur leurs coups sûrs.

\begin{table}[H]
\centering
\caption{Top 6 joueurs surpayés par rapport à leurs coups sûrs}
\label{tab:outliers}
\small
\begin{tabular}{lrrrr}
\toprule
...
\midrule
Andre Dawson    & 8.102 & 7.445 & 0.657 & 153 \\
Steve Buchele   & 7.863 & 6.810 & 1.053 & 111 \\
Kal Daniels     & 7.825 & 6.871 & 0.954 & 115 \\
Shawon Dunston  & 7.814 & 7.068 & 0.747 & 128 \\
Mark Grace      & 7.746 & 6.869 & 0.877 & 169 \\
...
Sammy Sosa (résidu $= -0.807$, 64 coups sûrs) en début de carrière.
...
Ces résultats sont cohérents avec la littérature en économie du sport et permettent
de comprendre pourquoi l'Association des joueurs a pu obtenir gain de cause dans le
grief concernant la sous-rémunération des agents libres en 1991/92 : la régression
multiple met en évidence un coefficient négatif pour les agents libres récents,
attestant d'une entente implicite entre clubs.\vspace{0.3cm}

Pour des travaux futurs, il serait pertinent d'intégrer les données de plusieurs
saisons consécutives afin d'étudier l'évolution des salaires, ou d'utiliser des
méthodes non linéaires (forêts aléatoires, boosting) pour améliorer le pouvoir
prédictif du modèle.\vspace{1cm}
